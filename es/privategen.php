...
if (mysqli_num_rows($result) < 1 && $rankk != "5") {
echo '
<script language="javascript">
...
$result2 = mysqli_query($con, "SELECT * FROM `package` WHERE `id` = '$packages'") or die(mysqli_error($con));

if (mysqli_num_rows($result2) < 1 && $rankk != "5") {

echo '
<script language="javascript">
    window.location.href = "purchase?noaccess-private"
</script>
';
...
<head>
	...
	<title><?php echo $website ?> | Generador Privado</title>
		...
		<hr />
					...
					<ol class="breadcrumb bc-4" >
						...
						<li class="active">
									...
									<strong>Generador Privado</strong>
							</li>
							</ol><?php
						$generatorquery = "SELECT * FROM `privgen`";
					...
					while ($row = mysqli_fetch_array($result)) {
						...
						echo'
							<div class="col-sm-3"><a href="#">
														<center><div class="popover-secondary" data-toggle="popover" data-trigger="hover" data-placement="bottom" data-content="El generador privado elimina la cuenta cuando se genera, por lo que solo tu tendrás esta cuenta." data-original-title="Que es el Generador Privado?" style="
										  ...
										  "><font color="white">Generador Privado</font></div></center></a>
								<div  id="tile-id'. $id .'" class="tile-block tile-'. $color .'" style="border: 3px #ff4d4d ridge; border-radius: 1px 1px 10px 10px;">
								...
								<a href="#">
							'. $name .'
							<span>Genera tus '; if($name === "Steam"){echo
							'clé ';}
							else
							{echo 'cuentas ';} echo'!</span>
						</a>
					</div>
					...
					echo'
						...
						<input type="text" id="generator'.$row["id"].'" onclick="select_all(this)" class="text-center form-control" placeholder="usuario:contraseña" readonly />
						
						
					</div>
					
					<div class="tile-footer">
					<button type="button" id="generate'.$row["id"].'" class="btn btn-block">Generar</button>
						                        <a onclick="myFunction'.$row["id"].'()" id="copy'.$row["id"].'" data-clipboard-target="generator'.$row["id"].'" title="Copy" class="btn btn-block">Copiar</a>
                    </div>
                </div>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-header">
					...
					<h4 class="modal-title">Configurez couleur n°'.$id . '</h4>
				</div>
				
				<div class="modal-body">
			                <label>Color :</label></br>
          <select onchange="changeClass'. $id .'(this.value)" name="color" class="form-control" style="margin-bottom: 5px;" required>
<option value="tile-block tile-blue">Bleu</option>
<option value="tile-block tile-cyan">Cyan</option>
<option value="tile-block tile-red">Rouge</option>
<option value="tile-block tile-green">Vert</option>
<option value="tile-block tile-aqua">Aqua</option>
<option value="tile-block tile-purple">Violet</option>
<option value="tile-block tile-pink">Rose</option>
<option value="tile-block tile-orange">Orange</option>
<option value="tile-block tile-brown">Marron</option>
<option value="tile-block tile-plum">Prune</option>
<option value="tile-block tile-gray">Gris</option>
<option value="tile-block tile-black">Noir</option>
</select>
		...
		echo '
			...
			$(document).ready(function(){
				$("#generate'.$row["id"].'").click(function(){
				 $.get("include/privategen?generator=priv'.$row["id"].'&username='.$username.'", function(response){
