...
echo'
           ...
           $icon = "bg-info";
			...
			<div class="col-md-12">
			<label>Vous:</label></br>
			<textarea type="text" name="newreply" class="form-control" style="margin-bottom: 5px;" placeholder="Répondez ici." rows="10" required></textarea>
			
			<button type="submit" class="btn btn-success btn-large btn-block" style="margin-bottom: 5px;">Ajouter une réponse</button></br>
		</form></div>
		';}elseif($isclose == "1"){
         echo'
         <center><h2>Le ticket est fermé </h2></center>
		 ';
...
echo '
						<tr>
							...
							<td>'.  $row['date']  .'</td>
							<td>'; if($row['isclose'] == "1"){
							echo'<span class="badge">Ticket fermé</span>';}elseif($row['isclose'] == "0" && $row['isadminread'] == "0"){echo '
							<a href="admin-support?id='. $row['idticket'] .'" data-target="#message'.$row['id'].'" data-parent="#menu">
							<span class="badge badge-secondary">Nouvelle réponse</span>
							</a>
							';}elseif($row['isclose'] == "0" && $row['isadminread'] == "1"){
							echo'
							<a href="admin-support?id='. $row['idticket'] .'" data-target="#message'.$row['id'].'" data-parent="#menu">
							<span class="badge badge-info">En attente de réponse...</span>
							</a>
							';} echo'</td>
						</tr>
';}}
