...
<head>
	...
	<title><?php echo $website ?> | Generateur</title>
		...
		<hr />
					...
					<ol class="breadcrumb bc-4" >
						...
						<li class="active">
									...
									<strong>Generateur Privé</strong>
							</li>
							</ol><?php
					...
					while ($row = mysqli_fetch_array($result)) {
						...
						echo'
							...
							<center><div class="popover-secondary" data-toggle="popover" data-trigger="hover" data-placement="bottom" data-content="Le générateur privé supprime le compte/la clé lorsqu\'il est généré, donc seul vous avez ce compte/clé." data-original-title="Qu\'est-ce qu\'un générateur privé?" style="
										  ...
										  "><font color="white">Générateur Privé</font></div></center></a>
					...
					echo'
						...
						<input type="text" id="generator'.$row["id"].'" onclick="select_all(this)" class="text-center form-control" placeholder="pseudo/email:motdepasse" readonly />
					...
					<button type="button" id="generate'.$row["id"].'" class="btn btn-block">Générer</button>
						                        <a onclick="myFunction'.$row["id"].'()" id="copy'.$row["id"].'" data-clipboard-target="generator'.$row["id"].'" title="Copy" class="btn btn-block">Copier</a>
                    </div>
                </div>
