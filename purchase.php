...
$generations = $row['generations']." /day";
}

if($row['privgenerations'] == ""){
$privgenerations = "Unlimited";
}else{
$privgenerations = $row['privgenerations']." /day";
}
...
$buy = " buy-three";
			...
			<div class="pricing-grid agileits-shadow" style="margin-bottom: 100px;">
				...
				<div class="agileinfo-price '. $infoprice .'">
					<h3>'. $name .'</h3>
					<h5>You will be premium for '. $length .'</h5>
				</div>
				<div class="price-bg">
					<p class="price-label-'. $label .'">'. $value .'<span>'. $donnee[0] .'</span><i>'; if(isset($donnee[1])){ echo $donnee[1];} echo'</i>
					'; if($row["length"] == "Lifetime"){
					echo'/ One time</p>';
					}
					elseif($donnee2[0] !== "1"){
					echo '/'.$donnee2[0].' '.$donnee2[1].'s</p>';}else{
					...
					<div class="buy'. $buy .'"> 
						<a href="#" onclick="myform'.$idpackage.'()" class="bg">Buy now </a>
					</div> </form>
				</div>
				</div>
